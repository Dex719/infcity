...
- [x] **TSK-142**: Тёмное стекло окон и витрин
  - Requirement: FR-19.20, AC-19.21
  - Deliverables: ключ `window` в `palette.ts`, `palette.json`, `palette.winter.json`; `windows()` и витрина `storefront` в `Buildings.ts`; оракул стекла в `tests/render/tone.test.ts`; тесты окон в `tests/scene/facade.test.ts`, `tests/scene/hidden.test.ts`; потолок палитры в `tests/config.test.ts`
  - Acceptance: AC-19.21; число вершин кварталов и снапшоты генератора без изменений
  - Факт: ключ `window` `#333648` (лето и зима), 28-й в палитре; `windows()` без параметра цвета — все проёмы и витрины торговых рядов одного стекла. Оракул: на свету (38, 41, 50) при референсе (37, 39, 48), в тени (13, 20, 28); стекло ≤ 0,4 яркости любой стены (кирпич — 0,35 летом, 0,38 зимой; прежнее синее — 0,73). В кадре тот же пиксель окна панельного дома: было (48, 87, 122), стало (38, 41, 50), стена рядом не изменилась; первый порог AC (0,35) оказался впритык к кирпичу — поднят до 0,4 с цифрами. Вершин не добавилось; unit 358/358

- [x] **TSK-143**: QA волны 8
  - Requirement: FR-19.8, FR-19.9
  - Depends on: TSK-142
  - Deliverables: замер окон в кадре против референса, эталоны, e2e, perf, qa-evidence
  - Acceptance: unit и e2e зелёные; пик ≤ 400 000 треугольников, ≤ 300 draw calls
  - Факт: проём в кадре (38, 41, 50) против референса (37, 39, 48); эталоны перезаписаны — изменились только проёмы и витрины во всех 20 кадрах (0,46…3,4 %), шума окружения нет; e2e 36/36, perf 326 374 / 126 / 79 FPS (пик треугольников тот же до единицы, FPS — тренд окружения); unit 358/358

#### Волна 9 (2026-09-24): кровли ТЦ

- [x] **TSK-144**: Кровля ТЦ — кондиционеры и световые фонари
  - Requirement: FR-19.21, AC-19.22
  - Deliverables: раскладка кровли в `Buildings.mall` (`src/scene/procedural/Buildings.ts`); тест в `tests/scene/facade.test.ts`
  - Acceptance: AC-19.22; снапшоты генератора без изменений
  - Факт: `Buildings.mallRoof` — 4 фонаря-пирамиды на бортиках и 2 ряда по 4 кондиционера с вентиляторами, у каждого ореол AO кровли (ширина 1); `roofDetails` зовётся с полосой у фасада `{ z: f.z + s·8, w: 41, d: 7 }`. Тест AC-19.22 для обеих видимых сторон: 8 вентиляторов и 4 фонаря внутри кровли, мимо полосы, у каждого 4 внутренних угла ореола; шпион подтверждает полосу, а следующее значение `rng` после `mall()` совпадает с одиночным `roofDetails`. Первая версия теста мерила центр пирамиды средним вершин — дубли на шве конуса смещали его на 0,19, дальний угол ореола уходил за радиус; исправлено на центр габарита. Снимки: крыши обоих ТЦ стартового кадра заполнены рядами установок; unit 361/361

- [x] **TSK-145**: QA волны 9
  - Requirement: FR-19.8, FR-19.9
  - Depends on: TSK-144
  - Deliverables: снимки ТЦ, эталоны, e2e, perf, qa-evidence
  - Acceptance: unit и e2e зелёные; пик ≤ 400 000 треугольников, ≤ 300 draw calls
  - Факт: квартал ТЦ — максимум 4 128 вершин из 7 000 (1 668 кварталов); эталоны — изменились 11 кадров, где виден ТЦ, мелкие различия в кадрах реки и Хан Шатыра — край крыши ТЦ у рамки; e2e 36/36, perf 328 374 / 127 / 101 FPS; unit 361/361

#### Волна 10 (2026-09-24): остальные большие кровли

- [x] **TSK-146**: Кондиционеры на кровлях торговых рядов и учебного корпуса
  - Requirement: FR-19.22, AC-19.23
  - Deliverables: `hvacUnit` и кровли в `Buildings.shopRow`, `Buildings.campusHall`, `Buildings.mallRoof` (`src/scene/procedural/Buildings.ts`); тесты в `tests/scene/facade.test.ts`
  - Acceptance: AC-19.23; геометрия ТЦ и снапшоты генератора без изменений
  - Факт: `hvacUnit` (корпус, вентилятор, ореол) вынесен из `mallRoof`, ТЦ собирается им в прежнем порядке; торговый ряд — ⌊(L − 8) / 8⌋ + 1 кондиционеров глубиной 1,6 за вывеской, `roofDetails` полосой перед ними; учебный корпус — 4 кондиционера по бокам купола. Тесты AC-19.23 для обеих видимых сторон: 3 и 5 установок на рядах 26 и 42, 4 на корпусе, у каждой 4 внутренних угла ореола, шпион и `rng` — как у ТЦ; тесты кровли ТЦ после выноса зелёные. Помощник теста сразу брал центр вентилятора центром габарита, а не первой вершиной (урок волны 9); unit 366/366

- [x] **TSK-147**: QA волны 10
  - Requirement: FR-19.8, FR-19.9
  - Depends on: TSK-146
  - Deliverables: снимки кварталов, бюджет перебором, эталоны, e2e, perf, qa-evidence
  - Acceptance: unit и e2e зелёные; пик ≤ 400 000 треугольников, ≤ 300 draw calls
  - Факт: бюджет — торговый 4 586, учебный 4 196, ТЦ 4 128 из 7 000; эталоны — изменились 14 кадров с рядами и корпусами (≤ 0,76 %), кадры только с ТЦ — ни на пиксель; e2e 36/36, perf 324 381 / 126 / 105 FPS (пик гуляет ± 6 тыс. от маршрута); unit 366/366

#### Волна 11 (2026-09-24): пояса-плиты новостроек

- [x] **TSK-148**: Пояса-плиты на фасадах новостроек
  - Requirement: FR-19.23, AC-19.24
  - Deliverables: `floorSlabs` в `Buildings.modernTower` (`src/scene/procedural/Buildings.ts`); тест в `tests/scene/facade.test.ts`
  - Acceptance: AC-19.24; снапшоты генератора без изменений
  - Факт: `Buildings.floorSlabs` — две белые плиты 0,2 с выносом 0,5 на каждом междуэтажном уровне видимых сторон; тест AC-19.24 для 4 поворотов: 18 плит у башни в 10 этажей, все вершины снаружи корпуса на видимой стороне, объёмы попарно не пересекаются (в углу плиты только соприкасаются), верх балконной плиты выше пояса на 0,02. Снимок: фасады новостроек разделены светлыми поясами с тенью под каждым, полоса лоджий поделена по этажам; unit 370/370

- [x] **TSK-149**: QA волны 11
  - Requirement: FR-19.8, FR-19.9
  - Depends on: TSK-148
  - Deliverables: снимки новостроек, бюджет перебором, эталоны, e2e, perf, qa-evidence
  - Acceptance: unit и e2e зелёные; пик ≤ 400 000 треугольников, ≤ 300 draw calls
  - Факт: квартал новостроек — максимум 6 172 из 7 000 (2 130 кварталов); эталоны — изменились 20 кадров, карта различий — ровно пояса и край их тени; e2e 36/36, perf 328 545 / 126 / 108 FPS; unit 370/370

#### Волна 12 (2026-09-24): вода в русле

- [x] **TSK-150**: AO русла — стенки набережных и полосы на воде
  - Requirement: FR-19.24, AC-19.25
  - Deliverables: стенки `boxAo` и ореолы воды в раскладке `river` (`src/scene/procedural/BlockPrefabs.ts`); тест в `tests/scene/ao.test.ts`
  - Acceptance: AC-19.25; снапшоты генератора без изменений
  - Факт: стенки набережных — `boxAo` (у уреза 0,50, наверху 0,88); на воде ореолы в сторону русла: южный 2,5 от стенки, северный 4,9 — от стенки под настилом набережной и ещё 2,5 за его краем. Первый вариант клал северную полосу на 2,5 от стенки — целиком под настил (`RIVER_DECK_EDGE` 21,8): в кадре изменился пиксель у парапета, остальное нет; замер показал промах, полоса продлена за край настила (константы `RIVER_WALL_INNER`, `RIVER_DECK_EDGE`). Речной квартал не поворачивается (328 из 328 с поворотом 0) — берега в мире постоянны. Снимок: вдоль северной набережной вода мягко темнеет к настилу. Тест AC-19.25: грань стенки к воде у уреза ≤ 0,55, наверху ≥ 0,85, полосы на всю длину воды; unit 372/372

- [x] **TSK-151**: QA волны 12
  - Requirement: FR-19.8, FR-19.9
  - Depends on: TSK-150
  - Deliverables: снимки реки, эталоны, e2e, perf, qa-evidence
  - Acceptance: unit и e2e зелёные; пик ≤ 400 000 треугольников, ≤ 300 draw calls
  - Факт: эталоны — изменились 5 кадров с рекой (≤ 4 %, разница до 57 — полосы AO); e2e 36/36, perf 327 640 / 125 / 107 FPS; unit 372/372

#### Волна 13 (2026-09-24): зелень деловой площади

- [x] **TSK-152**: Газонные острова с деревьями на деловой площади
  - Requirement: FR-19.25, AC-19.26
  - Deliverables: `BUSINESS_LAWNS` и острова в раскладке `businessGlass` (`src/scene/procedural/BlockPrefabs.ts`); тест в `tests/scene/layout.test.ts`
  - Acceptance: AC-19.26; снапшоты генератора без изменений
  - Факт: `BUSINESS_LAWNS` — три острова (восточный 12 × 12 с двумя деревьями, южный 7 × 8 и западный 5,5 × 16 — по дереву) на покрытии + 0,06, швы мощения их обходят; тест AC-19.26 по всем деловым кварталам окна: шпионы собирают башни (с ореолом), фонтан, скамейки, клумбы, велопарковку, столбики и асфальт парковки из настоящей раскладки — острова ни с чем не пересекаются, кроны в плите и мимо башен, швов внутри островов нет. Стартовый кадр: зелень 5,6 → 7,1 %, мощение 10,3 → 9,2 %; средний зум — зелень 6,2 → 8,0 %; unit 374/374

- [x] **TSK-153**: QA волны 13
  - Requirement: FR-19.8, FR-19.9
  - Depends on: TSK-152
  - Deliverables: замер зелени стартового кадра, бюджет перебором, эталоны, e2e, perf, qa-evidence
  - Acceptance: unit и e2e зелёные; пик ≤ 400 000 треугольников, ≤ 300 draw calls
  - Факт: зелень стартового кадра 5,6 → 7,1 %; деловой квартал — максимум 5 526 из 7 000 (1 485 кварталов); эталоны — изменились 13 кадров с деловыми площадями; e2e 36/36, perf 330 633 / 126 / 107,5 FPS; unit 374/374

#### Волна 14 (2026-09-24): зелень рынка

- [x] **TSK-154**: Газонные полосы с деревьями по краям рынка
  - Requirement: FR-19.26, AC-19.27
  - Deliverables: `MARKET_LAWNS` и полосы в раскладке `market`, перенос doc-комментария `river()` (`src/scene/procedural/BlockPrefabs.ts`); тест в `tests/scene/layout.test.ts`
  - Acceptance: AC-19.27; снапшоты генератора без изменений
  - Факт: `MARKET_LAWNS` — восточная полоса 8 × 22 (под двумя прежними деревьями + новое (19,5; −1)) и западная 5 × 19 (под кустами + новое (−20,5; 12)); общий помощник `lawnIslands` — им же собирает острова деловая площадь (геометрия та же); doc-комментарий `river()` снова над функцией. Тест AC-19.27: шпионы берут павильон (с ореолом), лотки (с ореолом), клумбу и столбики из раскладки — полосы мимо, на полосах 4 дерева, кроны новых — в плите, не касаются соседних крон и крыш лотков. Зелень стартового кадра 7,1 → 7,6 %; unit 376/376

- [x] **TSK-155**: QA волны 14
  - Requirement: FR-19.8, FR-19.9
  - Depends on: TSK-154
  - Deliverables: замер зелени стартового кадра, эталоны, e2e, perf, qa-evidence
  - Acceptance: unit и e2e зелёные; пик ≤ 400 000 треугольников, ≤ 300 draw calls
  - Факт: зелень стартового кадра 7,1 → 7,6 %; рынок — максимум 2 462 из 7 000 (1 032 квартала); эталоны — изменились 14 кадров с рынком; e2e 36/36, perf 331 735 / 125 / 107 FPS; unit 376/376

#### Обход после волн 6–14 (2026-09-24)

- [x] **TSK-156**: Визуальный обход изменённых типов кварталов при всех поворотах
  - Requirement: FR-19.8
  - Deliverables: снимки обхода (scratchpad сессии), запись в qa-evidence
  - Acceptance: каждый изменённый тип — при каждом из 4 поворотов на минимальной высоте камеры; найденные дефекты — в bugfix
  - Факт: 7 типов (торговый, ТЦ, учебный, новостройки, деловой, рынок, панельный) × 4 поворота = 28 кадров на высоте 60, склейки 2 × 2; дефектов нет — деревья не прорастают сквозь стены, установки кровли не наезжают на вывески и друг на друга, газоны не заходят на асфальт и мебель, фасады, маркизы, террасы и острова на видимых сторонах при каждом повороте

#### FR-20 (2026-09-24): зелёный старт

- [x] **TSK-157**: Сквер в центре старта и версия генератора 4
  - Requirement: FR-20, AC-20.1
  - Deliverables: `LANDMARKS.START_PARK` и `GEN.VERSION` 4 в `src/config.ts`; правило в `Generator.computeBlockType` (`src/world/Generator.ts`); тест AC-20.1 и обновлённый снапшот в `tests/world/generator.test.ts`
  - Acceptance: AC-20.1; AC-3.1, AC-3.3, AC-4.1 зелёные
  - Факт: правило в `computeBlockType` после русла и ландмарков; тест AC-20.1 — 1 000 seed: центр — парк без ландмарка, Байтерек и Хан Шатыр на местах, ни один из 8 соседей центра не парк; `GEN.VERSION` 4. Снапшот окна 21 × 21 обновлён намеренно (одна строка отпечатка); сравнение со старым кодом по 5 seed × 441 клетке — изменились 14 типов из 2 205 (центр, соседи, в одном seed — цепочка ещё на две клетки), повороты и варианты те же. Во встроенном браузере ссылка `?seed=astana&v=3` открывает новый город со статусом «Город обновился», ошибок в консоли нет; unit 378/378

- [x] **TSK-158**: QA зелёного старта
  - Requirement: FR-20, AC-20.2, FR-19.8
  - Depends on: TSK-157
  - Deliverables: замер 12 seed, эталоны, e2e, perf, qa-evidence, README
  - Acceptance: AC-20.2; unit и e2e зелёные; пик ≤ 400 000 треугольников, ≤ 300 draw calls
  - Факт: AC-20.2 — средняя 15,2 %, худший 10,1 % (порог 14,5 / 9); эталоны — изменились 6 кадров с центром старта, остальные 15 — ни на пиксель; e2e 36/36, perf 337 226 / 126 / 101 FPS; unit 378/378

#### FR-20.4 (2026-09-24): бульвар Нуржол

- [x] **TSK-159**: Раскладка бульвара Нуржол для парка центра старта
  - Requirement: FR-20.4, AC-20.3
  - Deliverables: `boulevard` и выбор раскладки в `buildBlock` (`src/scene/procedural/BlockPrefabs.ts`); тест в `tests/scene/layout.test.ts`
  - Acceptance: AC-20.3; снапшоты генератора без изменений
  - Факт: `NURZHOL` и `boulevard` — зелёная срединная полоса с фонтанами (центральный на круглой площади) и водными каналами, мощёные дорожки по бокам, тенистые ряды деревьев, скамейки и фонари в просветах, клумбы и кусты на газонах; `buildBlock` строит её у `LANDMARKS.START_PARK`, генератор не менялся. Первый вариант (аллея шириной 12) опустил зелень стартового кадра до 13,6 % — переделан по замеру до 14,9 % (design D30). Тест AC-20.3 при 4 поворотах: дорожки вдоль мировой оси, 3 фонтана на оси с каналами от чаши до чаши, ряды по 7 деревьев, кроны над дорожками, но не над срединной полосой, фонари не под кронами, швы только на дорожках; обычный парк — прежний; квартал ≤ 5 048 вершин; unit 383/383

- [x] **TSK-160**: QA бульвара
  - Requirement: FR-20.4, FR-19.8
  - Depends on: TSK-159
  - Deliverables: снимки центра старта при 4 поворотах, замер зелени 12 seed, эталоны, e2e, perf, qa-evidence, README
  - Acceptance: unit и e2e зелёные; средняя зелень стартового кадра ≥ 14,5 %; пик ≤ 400 000 треугольников, ≤ 300 draw calls
  - Факт: зелень стартового кадра 14,9 % (худший 10,0 %), снимки 12 seed — все 4 поворота центра; эталоны — изменились 6 кадров с центром старта, остальные 15 — ни на пиксель; e2e 36/36, perf 337 226 / 126 / 104,5 FPS; unit 383/383

#### Волна 15 (2026-09-25): пешеходные переходы

- [x] **TSK-161**: Зебры вне зоны перекрёстка и стоп-линия, у которой стоят машины
  - Requirement: FR-19.27, AC-19.28
  - Deliverables: `CROSSWALK` в `src/config.ts`; зебры, стоп-линии, пунктир, краевые линии и стрелки в `src/scene/procedural/Roads.ts`; запас торможения у стоп-линии в `src/mobs/Car.ts`; тесты в `tests/scene/street.test.ts` и `tests/mobs/traffic.test.ts`
  - Acceptance: AC-19.28; симуляция 2 и 3 минуты — 0 пересечений, 0 заторов; снапшоты генератора без изменений
  - Факт: `crosswalks` — 4 зебры по 8 полос `planeXZ` (у своей зоны и у дальних концов дорог), `stopLines` — на подъезжающих полосах в 0,8 от зебр, пунктир и краевые линии обрываются у переходов, стрелки — в 7,5 от зоны; `stopMargin` в `Traffic` — машина до стоп-линии встаёт в 4 от зоны, проехавшая её — у края зоны. Тесты AC-19.28 на обычном, речном, под ЛРТ и с площадью на углу: зебры вне зоны и внутри чанка, поперёк 93 % проезжей части; стоп-линия между зеброй (зазор 0,8) и передом стоящей машины; пунктир, краевые линии и стрелки их не касаются; разметка 800 вершин вместо 1 200. Трафик: машина с полной скорости встаёт передом в 4,0 от зоны; мутант со старым запасом 0,6 этот тест роняет. Симуляция 2 и 3 минуты и временный прогон 8 seed × 4 минуты со сдвигами окна — 0 пересечений, 0 заторов; unit 399/399

- [x] **TSK-162**: QA волны 15
  - Requirement: FR-19.8, FR-19.9
  - Depends on: TSK-161
  - Deliverables: крупные кадры перекрёстков (обычный, речной, под ЛРТ), эталоны, e2e, perf, qa-evidence, README
  - Acceptance: unit и e2e зелёные; пик ≤ 400 000 треугольников, ≤ 300 draw calls
...
- [x] **TSK-164**: QA волны 16
  ...
  - Факт: карточка — Байтерек целиком и бульвар Нуржол (сдвиг выбран из 4 вариантов); значок 32 px читается; e2e 37/37 без перезаписи эталонов, perf 327 624 / 126; unit 403/403

#### Волна 17 (2026-09-25): дворы

- [x] **TSK-165**: Детская площадка и газон двора новостройки
  - Requirement: FR-19.29, AC-19.30
  - Deliverables: `Props.playground` (`src/scene/procedural/Props.ts`); газон, площадка и скамейка в `residentialNew`, обход площадки деревьями (`src/scene/procedural/BlockPrefabs.ts`); тесты в `tests/scene/props.test.ts` и `tests/scene/layout.test.ts`
  - Acceptance: AC-19.30; снапшоты генератора без изменений
  - Факт: `Props.playground(x, z, facing)` — песок, домик `gold` с крышей-пирамидой `accent-red`, горка `flag-blue`, качели `steel` с сиденьями `accent-red`, карусель `yellow`; 339 вершин. `NEW_BUILD_YARD` — газон 12 × 16 у юго-западного края двора новостройки, площадка (−17; 12) и скамейка; случайные деревья обходят площадку с запасом 3,3. Горка — к видимой стороне квартала (`visibleZ(hidden)`): первый крупный кадр при повороте 2 показал её за домиком. Перебор 4 сида × 61²: новостройка — максимум 6 563 из 7 000 (2 129 кварталов), панельный — 6 494 (1 041). Тесты: площадка (пределы, высота, элементы, зеркало при −1) и двор новостройки (газон мимо башен с ореолами и машин, площадка мимо клумбы и фонарей, кроны деревьев не над площадкой, ≤ 7 000); unit 408/408

- [x] **TSK-166**: QA волны 17
  - Requirement: FR-19.8, FR-19.9
  - Depends on: TSK-165
  - Deliverables: крупные кадры дворов (панельный и новостройка, 4 поворота), замер зелени, эталоны, e2e, perf, qa-evidence, README
  - Acceptance: unit и e2e зелёные; пик ≤ 400 000 треугольников, ≤ 300 draw calls
  - Факт: крупные кадры дворов при 4 поворотах — площадка читается, горка к камере; зелень стартового кадра 14,9 → 15,2 %; эталоны — изменились 18 кадров только во дворах; e2e 37/37, perf 324 415 / 126 / 105 FPS; `og.jpg` пересобрана; unit 408/408

#### BUG-14 (2026-09-25): e2e управления в CI без видеокарты

- [ ] **TSK-167**: AC-1.2 и AC-8.2 — время приложения вместо настенного
  - Requirement: BUG-14 (bugfix.md), AC-1.2, AC-8.2
  - Deliverables: `e2e/controls.spec.ts` — `settledHeight` и цикл `simulate` в AC-1.2
...
| TSK-142 | Complete | 2026-09-24: тёмное стекло окон и витрин (`window`); unit 358/358 |
| TSK-143 | Complete | 2026-09-24: QA волны 8 — e2e 36/36, perf 326 374 / 126 / 79 FPS |
| TSK-144 | Complete | 2026-09-24: кровля ТЦ — кондиционеры и фонари; unit 361/361 |
| TSK-145 | Complete | 2026-09-24: QA волны 9 — e2e 36/36, perf 328 374 / 127 / 101 FPS |
| TSK-146 | Complete | 2026-09-24: кондиционеры на кровлях торговых рядов и учебного корпуса; unit 366/366 |
| TSK-147 | Complete | 2026-09-24: QA волны 10 — e2e 36/36, perf 324 381 / 126 / 105 FPS |
| TSK-148 | Complete | 2026-09-24: пояса-плиты новостроек; unit 370/370 |
| TSK-149 | Complete | 2026-09-24: QA волны 11 — e2e 36/36, perf 328 545 / 126 / 108 FPS |
| TSK-150 | Complete | 2026-09-24: AO русла — стенки и полосы на воде; unit 372/372 |
| TSK-151 | Complete | 2026-09-24: QA волны 12 — e2e 36/36, perf 327 640 / 125 / 107 FPS |
| TSK-152 | Complete | 2026-09-24: газонные острова деловой площади; unit 374/374 |
| TSK-153 | Complete | 2026-09-24: QA волны 13 — e2e 36/36, perf 330 633 / 126 / 107,5 FPS |
| TSK-154 | Complete | 2026-09-24: газонные полосы рынка; unit 376/376 |
| TSK-155 | Complete | 2026-09-24: QA волны 14 — e2e 36/36, perf 331 735 / 125 / 107 FPS |
| TSK-156 | Complete | 2026-09-24: обход 28 кадров после волн 6–14 — дефектов нет |
| TSK-157 | Complete | 2026-09-24: сквер в центре старта, `GEN.VERSION` 4; unit 378/378 |
| TSK-158 | Complete | 2026-09-24: QA зелёного старта — 15,2 % зелени, e2e 36/36, perf 337 226 / 126 / 101 FPS |
| TSK-159 | Complete | 2026-09-24: раскладка бульвара Нуржол; unit 383/383 |
...
| TSK-164 | Complete | 2026-09-25: QA волны 16 — e2e 37/37, эталоны без изменений |
...
| TSK-167 | In Progress | 2026-09-25: e2e управления на времени приложения |
