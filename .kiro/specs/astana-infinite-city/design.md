...
### D14: LOD как отдельный слой деталей (FR-18.9, FR-18.10)
...
**Кровля (FR-19.11).** По оракулу: `roof-dark` `#4a4f57` даёт 74 на свету, референс ≈ 134. Ключ `roof-dark` используется не только кровлей (урны, корпуса светофоров, тёмные детали крыш), поэтому кровля получает свой цвет через `m.shade('concrete', k)` — `concrete` (`#a8a89f`, 158 на свету) с множителем под цель 110…140; парапеты остаются акцентами. Проверка — оракул (AC-19.12) и контраст с каждым цветом парапета ≥ 40.
