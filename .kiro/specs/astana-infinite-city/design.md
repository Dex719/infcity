...
После волны 9 кровли ТЦ оборудованы, а соседние большие плоские кровли — пустые: у торговых рядов (42 × 11 и 26 × 11) на крыше только вывеска, у учебного корпуса (36 × 12) — купол посреди пустой кровли, и `roofDetails` он не вызывает вовсе.

**D24: общий кондиционер `hvacUnit` (FR-19.22).** Корпус `panel-grey` 3 × 1,3 × `depth`, вентилятор `cone8` цвета `black`, ореол AO кровли шириной 1 — вынесены из `mallRoof` в `hvacUnit(x, z, top, depth = 2)`; ТЦ собирается им же, его геометрия не меняется. Торговый ряд (оси как у ТЦ: `v` от фасада вглубь, `z = f.z − s·v`, ряд глубиной 11): ⌊(L − 8) / 8⌋ + 1 кондиционеров глубиной 1,6 с шагом 8 на `v = 3` (с ореолом — `v ∈ [1,2; 4,8]`, задняя кромка 5,5); `roofDetails` — полосой `{ z: v = −1,5, w: L − 1, d: 7 }`, детали в `v ∈ [−3,4; 0,4]` с запасом на размер — мимо вывески (`v = −4,9`) и ряда установок. Учебный корпус: четыре кондиционера на `v = 2,5`, `x = ±8, ±14` — по бокам купола (r 3,2), ореолы до `v = 4,5` при кромке 6. Бюджет: торговый квартал +8 установок × 67 ≈ +540 вершин (максимум перебора 4 050 → ≈ 4 590), учебный +4 × 67 ≈ +270 (3 928 → ≈ 4 200).

### Волна 11 (2026-09-24): пояса-плиты новостроек

Новостройки — ровные светлые стены с сеткой тёмных проёмов, вертикальной полосой лоджий и четырьмя балконами внизу: горизонтального ритма нет. У референса новые дома читаются поясами-плитами на каждом этаже (зелёный дом среднего плана). Панельные дома не трогаем: запас панельного квартала — 769 вершин на четыре дома, а ровный фасад для панельки честен.

**D25: пояса-плиты `floorSlabs` (FR-19.23).** На уровнях `i · FLOOR`, i = 1…floors − 1 (ниже — цоколь, наверху — карниз), две плиты `white` высотой 0,2 по центру уровня с выносом 0,5 на видимых сторонах (D13): плита стороны Z — от скрытого торца (+0,3 за угол) до видимого угла включительно (+0,5 — до внешней грани плиты стороны X), плита стороны X — от скрытого торца до стены Z, где упирается в плиту Z. Объёмы плит соприкасаются, но не пересекаются, поэтому в углу нет двойных граней. Балконная плита стоит на уровне `i · FLOOR` … `+0,12`, верх пояса — `+0,1`: верх балкона выше на 0,02, низ балкона внутри пояса. Полоса лоджий (вынос 0,3) проходит внутри плиты стороны X — плиты делят её на этажи. Основной батч; 48 вершин на уровень, до 528 на башню в 12 этажей; квартал новостроек: максимум перебора 4 795 → ≤ 6 380 из 7 000.

Отвергнуто: (а) пояса и у панельных домов — +144 вершины на дом при запасе 769 на четыре; (б) плиты одним боксом на обе стороны — бокс закрыл бы угол корпуса и скрытые стороны, +50 % вершин без пользы; (в) пояса через этаж — ритм реже, чем у референса.

### Волна 12 (2026-09-24): вода в русле

Речные кварталы — 10 % всех (1 464 из 14 365 в переборе четырёх сидов), в кадрах реки и площади вода занимает до трети кадра. Вода — плоскость одного цвета `water`, стенки набережных — боксы без AO: вода выглядит нарисованной поверх, а не лежащей в русле, хотя у всего остального в городе есть контактная тень (FR-19.1, FR-19.2).

**D26: AO русла (FR-19.24).** Стенки набережной (бокс 60 × 2,35 × 0,8 от −2,275 до +0,075 при воде на `RIVER.WATER_Y` = −2) строятся `boxAo`: затемнение от основания на `AO.WALL_HEIGHT` — у уреза (0,275 над основанием) множитель 0,50, наверху 0,88; стенка целиком в зоне затемнения, как нижний этаж дома. На воде вдоль каждой стенки — ореол `halo` с шириной только в сторону воды, цвет `water`, у стенки `AO.GROUND_MIN`, на высоте воды + `AO.GROUND_LIFT`; остальные стороны ореола нулевые. Геометрия берегов в мире постоянна — речной квартал не поворачивается (перебор: 328 из 328 с поворотом 0), камера смотрит с юга. Северная набережная — настил над водой от стенки (`RIVER_WALL_INNER` 24,2) до парапета (`RIVER_DECK_EDGE` 21,8): полоса шириной 2,5 от стенки целиком ушла бы под настил (первый вариант так и сделал — в кадре изменился едва один пиксель у парапета), поэтому северная полоса — 2,4 + 2,5 = 4,9: под настилом (его видно с низкой камеры, на 60 — на 4 вглубь) темнеет к стенке, а за краем настила, где его видно всегда, светлеет от 0,69 до 1 без шва. Южная полоса — 2,5 от стенки; первые ≈ 1,7 закрывает сама стенка, в кадре — её светлый хвост. Лодки — без ореола: они повёрнуты, а ореол выровнен по осям; тень на воду им даёт солнце. +16 вершин на квартал (максимум перебора 320).

### Волна 13 (2026-09-24): зелень деловой площади

Замер доли «зелёных» пикселей (g выше r и b на 25+): у референса в стартовом кадре зелень — 15,7 %, бежевое мощение — 2,1 %; у нас — 5,6 % и 10,3 %. Какие кварталы в кадре — решает генератор (снапшоты не трогаем), а центр нашего стартового кадра — деловой квартал: мощёная площадь `stone-light`, газонная вставка у него есть, но на северном краю за главной башней — при повороте 0 её закрывает башня.

**D27: газонные острова `BUSINESS_LAWNS` (FR-19.25).** Три острова `grass` на покрытии + 0,06 (как вставка, BUG-7): восточный `(14,5; −4)` 12 × 12 — между башнями (главная кончается на x ≤ 4,5, её ореол — на 7; пристройка и её ореол начинаются с z 3,5; парковка — z ≤ −13), южный `(0,5; 16)` 7 × 8 — между скамейками (x ≤ −5,1) и ореолом пристройки (x ≥ 4,5), севернее столбиков (z 22,5) и южнее клумбы и велопарковки (z ≤ 10,4); западный `(−20,25; −4)` 5,5 × 16 — между ореолом главной башни (x ≥ −17 при наибольшей ширине 19) и краем плиты. Деревья лиственные: восточный — (12; −7) ×1,1 и (17; 0,5) ×1,0, южный — (0,5; 16) ×1,0, западный — (−20; −8) ×0,9; кроны (до 2,47 × масштаб от ствола) — внутри плиты и мимо корпусов. Острова добавлены в обход швов мощения. `rng` не вызывается: случайные деревья `scatterTrees` остаются, где были, и могут стоять на островах — это газон. Бюджет: 12 вершин газонов и 4 дерева; деловой квартал — максимум перебора 4 802 → ≈ 5 700 из 7 000.

Отвергнуто: (а) поменять типы кварталов стартового окна на парки — меняет генератор и снапшоты; (б) зеркалить острова по видимой стороне, как ТЦ (BUG-13), — площадь несимметрична (фонтан, парковка, пристройка), а со всех четырёх сторон острова видны и так: заслоняет их только главная башня по диагонали, а острова лежат по бокам от неё.

### Волна 14 (2026-09-24): зелень рынка

Рынок — второй после деловой площади бежевый участок стартового кадра (песочная площадь с лотками у левого края). Его боковые края свободны: на востоке на песке уже стоят два дерева (18; −14) ×1,1 и (20; −6) ×0,9 и клумба (17; 5), на западе — два куста (−21; 8) и (−21; 16) и фонарь.

**D28: газонные полосы `MARKET_LAWNS` (FR-19.26).** Восточная `(18,5; −9)` 8 × 22 — `x ∈ [14,5; 22,5]`, `z ∈ [−20; 2]`: павильон с ореолом кончается на x 9,5, лотки начинаются с z 4,5, клумба — с z 3,6, столбики — до x 10,3; новое дерево (19,5; −1) ×0,9 — крона до z 1,2, от кроны дерева (20; −6) её отделяют 0,6. Западная `(−20,5; 12,5)` 5 × 19 — `x ∈ [−23; −18]`, `z ∈ [3; 22]`: лотки кончаются на x −14,5, ореол павильона — на z 0,5; новое дерево (−20,5; 12) ×0,9 — между кустами. Газон на покрытии + 0,06, как вставки (BUG-7). `rng` не вызывается. +8 вершин газонов и 2 дерева; рынок — максимум перебора 2 106 → ≈ 2 500.

Попутно: константы берега `RIVER_WALL_INNER` и `RIVER_DECK_EDGE` (волна 12) были вставлены между doc-комментарием «Русло Есиль» и `river()` — комментарий описывал константу, а не функцию; константы перенесены выше комментария.

### FR-20 (2026-09-24): зелёный старт

**D29: сквер в центре старта (FR-20).** Чанк `LANDMARKS.START_PARK` (0, 0) — всегда `park`: правило в `Generator.computeBlockType` после русла и ландмарков, до стадиона. Русло там не бывает (ряды русла — `gy ≡ 6 mod 12`), ландмарк тоже (стартовая зона радиусом 4 запрещает случайные ландмарки, фиксированные — на (0, ±1)). Соседи центра не станут парком по прежнему правилу чётности: у клеток класса 1 исключены типы соседей класса 0, куда входит центр, у клеток класса 3 — всех соседей младших классов, а 9 регулярных типов 8 соседями не исчерпать. Затронуты центр и соседи, у которых раньше выпадал парк: они перевыбирают тип, а за ними изредка перевыбирает и следующая клетка цепочки чётности. Замер по 5 seed × окно 21 × 21: изменились 14 клеток из 2 205 (0,6 %) — центр, его соседи и в одном seed цепочка ещё на две клетки; повороты и варианты не меняются, остальной город — прежний. `GEN.VERSION` 3 → 4: снапшот окна 21 × 21 (`astana`) обновляется намеренно, ссылки с `v=3` открывают новый город с уведомлением «Город обновился» (`Shell`, как было).

Замер доли зелени стартового кадра по 12 seed (прототипы, откатаны до реализации):

| Вариант | Средняя | Худший | Медиана | Что меняет |
|---|---|---|---|---|
| Было | 10,4 % | 4,3 % | 11,5 % | — |
| А: парк в центре | 15,2 % | 10,1 % | 14,4 % | центр и его соседи |
| Б: вес парка левого берега 0,7 → 2 | 13,0 % | 7,7 % | 13,8 % | весь левый берег любого города |
| А + Б | 15,6 % | 12,2 % | 15,5 % | весь левый берег |

**Decision:** А. **Rationale:** одна точечная правка выводит старт на уровень референса (15,7 %); Б добавляет к средней 0,4 процентного пункта ценой перестановки кварталов всего левого берега во всех городах. Центр — обычный парк (его раскладка уже зелёная: газоны, аллеи, фонтан, деревья); отдельный тип «бульвар Нуржол» со своей раскладкой — возможное продолжение, не нужен для цели.

**D30: бульвар Нуржол в центре старта (FR-20.4).** Что взято у настоящего бульвара: пешеходная ось больше 2 км от Ак Орды через Байтерек к Хан Шатыру, шириной до 100 м, с аллеями, газонами, рядами деревьев, клумбами, скамейками и множеством фонтанов, включая каскадные; «водно-зелёный» — за фонтаны и зелень (welcome.kz, apartamenty.kz, ru.wikipedia «Бульвар Нуржол»). Генератор не меняется: чанк `LANDMARKS.START_PARK` остаётся `park`, а `buildBlock` строит у него `boulevard` вместо обычного `park`.

Раскладка без `rng`, в осях `along` (вдоль оси Байтерек — Хан Шатыр, мировая Z) и `across`: при чётном повороте квартала `along` — локальная Z, при нечётном — локальная X (поворот `rotation × 90°` переводит её в мировую Z); раскладка симметрична по `along`, поэтому направление оси неважно. Плита — газон. По оси — зелёная срединная полоса (`across` ±2,5) с фонтанами: центральный r 3,5 на круглой мощёной площади r 5,5, боковые r 2,2 на `along` ±14; между ними — водные каналы шириной 1,2 с белыми бортиками. По бокам — мощёные дорожки `stone-light` от 2,5 до 5 на всю длину 46, швы мощения — только на них и вне площади. Ряды деревьев на `across` ±6,8, `along` −19,5…19,5 с шагом 6,5, масштаб 1,1: кроны (до 2,72) нависают над дорожками, как тенистая аллея, но не над срединной полосой. Скамейки и фонари — в просветах между деревьями; на боковых газонах четыре клумбы и кусты по углам; столбики поперёк дорожек на концах.

Первый вариант — одна мощёная аллея шириной 12, деревья 0,9 вне аллеи, шесть клумб — опустил зелень стартового кадра по 12 seed до 13,6 % (обычный парк давал 15,2 %): мощение и клумбы заняли газон. Итерации по замеру: срединная полоса с двумя дорожками — 14,2 %; ряды деревьев ближе к оси — 14,4 %; дорожки 2,5 вместо 3,5, площадь 5,5 вместо 6,5, четыре клумбы — 14,9 % (худший seed 10,0 %), выше порога 14,5 %. Бюджет — ≈ 5 100 вершин из 7 000.

### Волна 15 (2026-09-25): пешеходные переходы

Сравнение перекрёстков крупным планом (вырезка ×3 из стартовых кадров): у референса с каждой стороны перекрёстка — 7–8 белых полос поперёк всей проезжей части, длина полосы — около четверти её ширины, ширина полосы — как зазор между ними. У нас — 5 полос 0,8 × 1,6: они перекрывают 72 % ширины и не доходят до бордюра квартала. Стоп-линия отстоит от конца зебры на 0,45 и в кадре сливается с ней в «гребёнку». Западная и северная зебры лежат внутри зоны перекрёстка, где едет поперечный поток. Машины уступают у края зоны (`STOP_MARGIN` 0,6 до −20): на восточном и южном подъездах они стоят передом на зебре, за стоп-линией.

**D31: зебры вне зоны и стоп-линия, у которой стоят машины (FR-19.27).** Размеры — в `CROSSWALK` (`src/config.ts`), их читают и разметка (`Roads`), и трафик (`Car`). Проезжая часть обеих дорог — от бордюра до бордюра, `[−28,5; −20]`: с одной стороны внешняя полоса тротуара `[−30; −28,5]`, с другой плита квартала с −20. Зебра — 8 полос `planeXZ` шириной 0,55 через 0,5 и длиной 2,2 вдоль движения, поперёк — `[−28,2; −20,3]` (93 % проезжей части), на высоте `MARK_Y` + 0,005, с отступом 0,3 от края зоны. Каждый чанк кладёт 4 зебры. У своей зоны — восточную (x ∈ [−19,7; −17,5] на дороге E–W) и южную (z ∈ [−19,7; −17,5] на дороге N–S). У дальних концов своих дорог — x ∈ [27,5; 29,7] и z ∈ [27,5; 29,7]: это западная и северная зебры зон соседей, их зона начинается на +30. Так все четыре зебры любой зоны лежат вне её, и ни одна не уезжает к соседу.

Стоп-линии — `box` шириной 0,5 на подъезжающей полосе: от оси дороги до 0,45 от бордюра (краевая линия кончается в 0,375 от него, линии не касаются), с зазором 0,8 до зебры. У своей зоны они лежат на северной полосе E–W (едет на запад) и восточной N–S (едет на север), x/z ∈ [−16,7; −16,2]; у дальних концов — на южной E–W (на восток) и западной N–S (на юг), x/z ∈ [26,2; 26,7]. Осевой пунктир идёт между стоп-линиями, в 0,5 от них: от −15,7 с шагом 6, пока штрих кончается до 25,7 (7 штрихов вместо 8). Краевые линии идут между зебрами, от −17,2 до 27,2. Стрелки отодвинуты от стоп-линий: центр на `blockMin` + 7,5, остриё стрелки северного подъезда — в 2,7 от стоп-линии.

Трафик: `CROSSWALK.STOP_SETBACK` = 4 — перед уступающей машины встаёт в 4 от края зоны, в 0,2 от стоп-линии. В `Car.sense` запас торможения выбирается по положению. Пока до зоны больше `STOP_SETBACK − 0,5`, машина тормозит до стоп-линии (`allowed = √(2a·(d − 4))`). Проехавшая стоп-линию машина, как раньше, тормозит до края зоны (`STOP_MARGIN` 0,6) — она не встаёт на перекрёстке. Допуск 0,5 больше шага кадра при ползучей скорости: машина, подползшая к линии вплотную, не перескакивает на запас 0,6 и не рвётся к зоне. Остальные правила — фазы, «не занимай перекрёсток», радар — не меняются: они считают расстояние до зоны. Тормозной путь с полной скорости — 15² / (2 · 27) ≈ 4,2, а перекрёсток машина начинает учитывать за 14 до зоны: до стоп-линии остаётся 10.

Отвергнуто: сдвинуть только разметку и оставить трафик — с длинными зебрами на зебре стояли бы машины всех четырёх подъездов, а не двух; сдвинуть саму зону перекрёстка — это меняет правила пересечений и фаз, а нужна только точка остановки.

Бюджет: полосы зебр — `planeXZ` по 4 вершины вместо боксов по 24, 4 × 8 × 4 = 128 вершин вместо 4 × 5 × 24 = 480 (−352 вершины и −176 треугольников на чанк); стоп-линии — прежние 96 вершин, пунктир — на штрих меньше.

### Волна 16 (2026-09-25): карточка ссылки и значок

У референса в `<head>` — Open Graph: `og:type`, `og:url`, `og:description` и `og:image` с абсолютным адресом картинки; пересланная ссылка разворачивается в карточку с кадром города. У нас — только `<title>` и `description`: ссылка из «Поделиться» приходит голой строкой, а вкладка — с пустым значком (сервер отдаёт 404 на `favicon.ico`).

**D32: карточка ссылки и значок (FR-19.28).** В `index.html` — `og:type` `website`, `og:site_name` и `og:title` «Astana Infinite», `og:locale` `ru_RU`, `og:description` — текст `description`, `og:url` — адрес Pages `https://dex719.github.io/infcity/`, `og:image` — `https://dex719.github.io/infcity/og.jpg` с `og:image:width` 1200, `og:image:height` 630, `og:image:type` `image/jpeg` и `og:image:alt`, `twitter:card` `summary_large_image`. Адреса абсолютные: краулеры не разрешают относительные пути ([dev.to](https://dev.to/grabbit/open-graph-image-sizes-and-dimensions-the-complete-2026-guide-1k16), [myog.social](https://myog.social/articles/og-image-size-guide)). 1200 × 630 — практический стандарт превью Facebook, X, LinkedIn, Telegram, WhatsApp и Slack, а не правило протокола. Сборка с относительным `base` не меняется: файлы из `public/` лежат в корне `dist/`, то есть по этим адресам.

Значок — `public/favicon.svg` 64 × 64: небо, полоса газона, светлый постамент, белый ствол Байтерека с чашей и гранёный золотой шар с жёлтой гранью — цвета `sky`, `grass`, `stone-light`, `white`, `gold`, `yellow` летней палитры, скруглённый квадрат. SVG-значки понимают Chrome и Edge с 80, Firefox с 41 и Safari только с 26 ([caniuse](https://caniuse.com/link-icon-svg)), поэтому рядом лежат `favicon-32.png` — запасной значок — и `apple-touch-icon.png` 180 × 180: для домашнего экрана iOS он без скругления, углы iOS скругляет сама. `theme-color` не зашит в HTML: `applyPalette` ставит его из неба палитры вместе с CSS-переменными, поэтому зимой он зимний.

PNG-значки и `og.jpg` собирает `tools/share-assets.js` — один скрипт на Node и Playwright (он уже в devDependencies). Скрипт поднимает `vite preview` над `dist/` через API Vite, рендерит значки из SVG в Chromium и снимает кадр 1200 × 630: стартовый вид seed `astana`, пауза, мобы пересозданы, заголовок убран. В стартовом кадре шар Байтерека срезан верхним краем, поэтому вид сдвинут на 290 пикселей вниз: Байтерек входит целиком, под ним — бульвар Нуржол с фонтанами. JPEG с качеством 85. Запуск: `npm run build && node tools/share-assets.js`; картинки коммитятся, как эталоны e2e.

### Волна 17 (2026-09-25): дворы

У референса кварталы плотно застроены, у нас двор новостройки — пустое светлое мощение вокруг трёх башен: газон только в юго-восточном углу, в остальном — фонарь, клумба и ряд машин. А детская площадка панельного двора — три коробки на песке (красный куб, синяя планка, жёлтый столбик) и в кадре читается как мусор. Во дворах Астаны — и панельных, и в новых ЖК — всегда есть площадка: домик с горкой, качели, карусель.

**D33: детская площадка и газон двора новостройки (FR-19.29).** `Props.playground(x, z)` — площадка 7 × 7 из песка на прежнем месте, остальное в её пределах:

| Элемент | Геометрия | Цвет | Вершин |
|---|---|---|---|
| Песок | `plane` 7 × 7 на 0,26 | `sand` | 4 |
| Домик | `box` 1,4 × 1,3 × 1,4 с центром (−2; −2) | `gold` | 24 |
| Крыша | `pyramid4` 2 × 0,9 × 2, повёрнута на 45° — стороны вдоль осей | `accent-red` | 19 |
| Горка | наклонная плита 0,8 × 0,1 от верха домика (+0,7 по Z) к земле на +3 | `flag-blue` | 24 |
| Качели | 2 стойки 0,12 × 2 на (−0,7…1,9; +2,2), перекладина на 2,26, 2 сиденья и 2 подвеса | `steel`, `accent-red` | 168 |
| Карусель | диск `cylinder8` r 1,1 на 0,55, ось и поручень | `yellow`, `steel` | 100 |

Итого 339 вершин вместо 76. Горка идёт на юг от домика и кончается на z +1, рама качелей — на z +2,2, карусель — в северо-восточной четверти: элементы не пересекаются. Высота — не больше 2,6 (перекладина качелей и конёк крыши). Горку от камеры закрывал бы домик, если она съезжает на скрытую сторону квартала (первый крупный кадр при повороте 2 так и показал — синяя полоса за домиком), поэтому раскладка передаёт `facing` — видимую сторону по Z (`−hidden.z`, D13): при поворотах 2 и 3 площадка зеркальна по Z.

Двор новостройки: газон `grass` на покрытии + 0,06 (вставка, BUG-7) `x ∈ [−23; −11]`, `z ∈ [3,5; 19,5]`, на нём площадка (−17; 12) и скамейка у её восточного края. Раскладка знает только диапазоны размеров башен, поэтому границы берутся с запасом. Третья башня стоит на (−1; 12) шириной до 14, то есть с x ≥ −8, её ореол — ещё 2,5, до −10,5. Первая кончается на z ≤ −3,5, её ореол — на −1. Клумба (−20; 5) остаётся на газоне, фонарь (−20; 20) и ряд машин на z 22 — на мощении. Случайные деревья (`scatterTrees`) получают площадку в список обхода: `rng` вызывается столько же раз, меняется только то, какие броски приняты, — это раскладка деревьев в самом квартале, а не генератор. Бюджет: новостройка — максимум перебора 6 172 + 4 + 339 + 48 ≈ 6 560, панельный квартал — 6 231 + 263 ≈ 6 500 из 7 000.
