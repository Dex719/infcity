...
маркизы магазинов и кафе-террасы под зонтиками, мягкая тень у ландмарков; белые мягкие облака
растворяются у низкой камеры, а их тени остаются. Тон подобран расчётной моделью освещения
и сверен с замером пикселей кадра.
...
попадают всё равно), поэтому кадр укладывается в 326 тысяч треугольников из 400 тысяч
и 126 draw calls из 300.
...
```bash
npm run test       # Vitest, один прогон (358 тестов)
