...
/**
 * Максимум вершин квартала каждого регулярного типа в окне 21×21 seed `astana` — нижняя
 * граница: падение ниже неё означает, что благоустройство, фасады или окна потеряны.
 * История: 2026-09-19 — замер сразу после отсечения скрытых сторон (FR-18.1), заменил
 * проверку роста относительно итерации 2 (AC-17.7). 2026-09-23 — перезамер после TSK-119
 * (FR-19.6): окна стали отдельными проёмами по 4 вершины вместо лент-боксов по 24, поэтому
 * `residential-new` похудел (4 516 → 4 231); остальные типы выросли на деталях итерации 4
 * и на AO итерации 5 (`boxAo`, ореолы).
 */
const AFTER_CULLING_BLOCK_MAX: Readonly<Record<string, number>> = {
  ...
  market: 2034,
  park: 3690,
  'residential-new': 4231,
  'residential-panel': 5520,
  river: 320,
  square: 2959,
  stadium: 2812,
};
...
/** Допуск вниз от таблицы: детали итераций 4–5 только добавляются (FR-18.6, FR-19). */
